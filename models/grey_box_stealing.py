...
def train(model_b, train_data_b, epochs=10):
  ...
  for iter in range(epochs):
    ...
    #   wandb.log({"Train Accuracy": 100*trainAcc})
    #   wandb.log({"Test Accuracy": 100*testAccu})
      # advAccu = evaluate_adversarial(model, test_dataset)
  # model.eval()
  return model_b
...
if __name__ == '__main__':
    ...
    args = args_parser()
    wandb.init(
        project="centralised-setting-adv-dp",
    ...
    target_model.load_state_dict(torch.load('watermarked_model_0.5.pt'))
    device = 'cuda'
    ...
    apply_transform = transforms.Compose([
                ...
                transforms.ToTensor()])
    data_dir = '../data/mnist/'
    train_dataset_b = datasets.MNIST(data_dir, train=True, download=True,transform=apply_transform)
    test_dataset_b = datasets.MNIST(data_dir, train=False, download=True,transform=apply_transform)
    samples_in_test = len(test_dataset_b)
    
    train_data_b, test_data_b = random_split(test_dataset_b, [200, samples_in_test-200])
    train_dataset_b_loader_grey = DataLoader(train_dataset_b, batch_size=32, shuffle=True)
    training_dataset_loader = DataLoader(train_data_b, batch_size=32, shuffle=True)
    load_watermark_dataset = torch.load('./watermarks/watermarks_samples_0.5_40.pt')
    water_data_b = DatasetSplit(load_watermark_dataset)
    ...
    dummy_labels = None
    dummy_labels = oracle(target_model, train_dataset_b, device)
    dummy_dataset = create_dataset(train_dataset_b, dummy_labels)
    model_b = train(model_b, dummy_dataset, epochs=1)
    for idx in range(7):
    # model_b.train()
        dummy_labels = None
    ...
    load_watermark_dataset1 = torch.load('./watermarks/watermarks_samples_0.4_40.pt')
    water_data_b1 = DatasetSplit(load_watermark_dataset1)
    waterAccur = evaluate_water(model_b, water_data_b1)

    load_watermark_dataset2 = torch.load('./watermarks/watermarks_samples_0.5_40.pt')
    water_data_b2 = DatasetSplit(load_watermark_dataset2)
    waterAccur = evaluate_water(model_b, water_data_b2)

    load_watermark_dataset3 = torch.load('./watermarks/watermarks_samples_0.6_40.pt')
    water_data_b3 = DatasetSplit(load_watermark_dataset3)
    waterAccur = evaluate_water(model_b, water_data_b3)

    torch.save(model_b.state_dict(), 'grey_box_stolen_watermarked_model_0.5_with_trainingD.pt')
