...
# lenet-5
class MNIST_CNN_SMALL_LENET(nn.Module):
    def __init__(self, input_size) -> None:
        ...
        if input_size==1:
            ...
            self.check = 8
        self.net = nn.Sequential(
            nn.Conv2d(input_size, 20, kernel_size=5, stride=1, padding=2), #Output: (32,32,20)
            nn.MaxPool2d(2, stride=2), #Output (16, 16, 20)
            nn.ReLU(inplace=True),

            nn.Conv2d(20, 50, kernel_size=5, stride=1, padding=2), #Output (16, 16, 50)
            nn.MaxPool2d(2, stride=2), #Output (8, 8, 50)
            nn.ReLU(inplace=True),

            nn.Flatten(),

            # nn.Linear(7 * 7* 50, 500),
            nn.Linear(self.check * self.check * 50, 500),
            nn.ReLU(inplace=True),
