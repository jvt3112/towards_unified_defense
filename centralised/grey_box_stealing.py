...
def mix_adv_samples(images, labels, model, pgd_eps=0.25, pgd_attack_steps=25, pgd_step_size=0.01, frac=1):
...
# training the surrogate model based on generated input-output pairs
def train(model_b, train_data_b, epochs=10):
  ...
  for iter in range(epochs):
      ...
      for batch_idx, (images, labels) in enumerate(training_dataset_loader):
          ...
          images = mix_adv_samples(images, labels, model_b)
          optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    torch.save(model_b.state_dict(), 'new_fas_adv_grey-box-stolen_watermarked_model_fashion_mnist_final.pt')
